(ns ocr-lib.core-helper
 (:require [clojure.java.io :refer [resource file writer]]
           [clojure.set :refer [intersection difference union select]])
 (:import [java.io File
                   IOException]
...
(defn clone-image
 ...
 (let [cm (.getColorModel image)
       ...
       copied-image (BufferedImage. cm raster false nil)]
  copied-image))
 ...
 (doseq [x (range width)]
  (doseq [y (range height)]
   (let [p (.getRGB image x y)
         ...
         r (contrast avg contrast-value)
         g (contrast avg contrast-value)
         b (contrast avg contrast-value)
...
(defn tacke-i-kvacice-ii
 ...
 [sve_tacke
  ...
  y-min
  y-max]
 (let [ispitna-povrsina (atom #{})]
  (doseq [x (range x-min x-max)]
   (doseq [y (range (- y-min 8) (dec y-min))]
    (swap! ispitna-povrsina conj [x y])))
  (doseq [x (range x-min x-max)]
   (doseq [y (range (inc y-max) (+ y-max 8))]
    (swap! ispitna-povrsina conj [x y])))
  (intersection sve_tacke @ispitna-povrsina))
 )

(defn dodaj-u-sortiran-skup
 ""
 ...
 (let [red (select
            (fn [[[y-min-p y-max-p] elem-p]]
             (let [y-skup-reda (set (range y-min-p y-max-p))
                   y-skup-znaka (set (range y-min y-max))]
              (not
               (empty?
                (intersection
                  y-skup-reda
  ...
  (if (empty? red)
   (conj skupovi_tacaka_slova
         [[y-min y-max] (conj element-reda
                              [x-min tacke_znaka])])
   (let [skupovi_tacaka_slova (difference
                                skupovi_tacaka_slova
                                red)
         prvi-red (first red)
         [[y-min-r y-max-r] elem-r] prvi-red
         y-min-d (atom
                  (if (< y-min
                         y-min-r)
                   y-min
    ...
    (doseq [[[y-min-s y-max-s] elem-s] red]
     (when (< y-min-s @y-min-d)
      (reset! y-min-d y-min-s))
     (when (< @y-max-d y-min-s)
      (reset! y-max-d y-max-s))
     (doseq [el-s elem-s]
      (swap! novi-red conj el-s))
     )
    (swap! novi-red conj [x-min tacke_znaka])
    (conj skupovi_tacaka_slova
          [[@y-min-d @y-max-d] @novi-red]))
   ))
 )
...
(defn grupisanje
 ...
 (if (empty? sve_tacke)
  ...
  (let [[x y] (first sve_tacke)
        ...
        [sve_tacke_nove tacke_znaka] (nadji-tacke-znaka
                                       sve_tacke
                                       #{[x y]}
                                       #{}
                                       width
                                       height)
        [x-min y-min x-max y-max] (find-min-max
                                    tacke_znaka)
        tacke-i-kvacice-set (tacke-i-kvacice-ii
                              sve_tacke_nove
                              x-min
                              x-max
                              y-min
                              y-max)
        [sve_tacke_nove tacke_znaka] (if (empty? tacke-i-kvacice-set)
        ...
        [x-min y-min x-max y-max] (find-min-max
                                    tacke_znaka)        
        skupovi_tacaka_slova (dodaj-u-sortiran-skup
                               skupovi_tacaka_slova
                               x-min
                               y-min
                               x-max
...
(defn proveri-poklapanja-fn
 ""
 [nulte-koordinate
  procitani_znakovi]
 (let [rezultat (atom {:znak ""
                       :poklapanja 0})
       znakovi_keys (keys procitani_znakovi)]
  (doseq [z_key znakovi_keys]
   (doseq [skup_tacaka (z_key procitani_znakovi)]
    (let [presek (count (intersection nulte-koordinate skup_tacaka))
          unija (count (union nulte-koordinate skup_tacaka))
          procenti_poklapanja (double
                               (/ (* presek
                                     100)
                                  unija))
          poklapanja (:poklapanja @rezultat)]
     (when (< poklapanja
              procenti_poklapanja)
      (swap!
        rezultat
        assoc
        :znak (name z_key)
        :poklapanja procenti_poklapanja))
     ))
   )
  @rezultat))

(defn procitaj-znakove
 ""
 [znakovi
  svetlost-value
  kontrast-value]
 (let [tacke_znakova (atom {})
       znakovi_keys (keys znakovi)]
  (doseq [z_key znakovi_keys]
   (let [vektor_slika_znaka (z_key znakovi)
         z_key_value (atom [])]
    (doseq [slika_byte_array vektor_slika_znaka]
     (let [slika_is (ByteArrayInputStream. slika_byte_array)
           slika (ImageIO/read slika_is)
           width (.getWidth slika)
           height (.getHeight slika)
           debug (grayscale-contrast
                   slika
                   width
                   height
                   svetlost-value
                   kontrast-value)
           sve_tacke (procitaj-crne-tacke slika width height)]
      (swap!
        z_key_value
        conj
        sve_tacke))
...
(defn procitaj-sliku-ii
 ""
 [image-byte-array
  svetlost-value
  kontrast-value
  znakovi]
 (let [image-is (ByteArrayInputStream. image-byte-array)
       ...
       debug (grayscale-contrast
               ...
               svetlost-value
               kontrast-value)
       sve_tacke (procitaj-crne-tacke image width height)
       svi_znakovi (grupisanje
                     sve_tacke
                     (sorted-set-by
                      (fn
                       [[[y-min1 y-max1] elem1]
                     ...
                     height)
       slike-znakova (atom [])
       procitani_znakovi (procitaj-znakove
                           znakovi
                           svetlost-value
                           kontrast-value)
       ]
  (doseq [[[y-min y-max] red] svi_znakovi]
   (doseq [[_ tacke_znaka] red]
    (try
     (let [nulte-koordinate (svedi-na-nulte-koordinate tacke_znaka)
           {znak :znak
            poklapanja :poklapanja} (proveri-poklapanja-fn
                                      nulte-koordinate
                                      procitani_znakovi)
 ...
 (let [tekst ["tekst.jpg" 3096 4128]
  ...
  (let [image (time (procitaj-sliku url width height))
   ...
   (time (doseq [[_ tacke_znaka] svi_znakovi]
          ...
          (try
           (let [nulte-koordinate (svedi-na-nulte-koordinate tacke_znaka)
                 fout (file (str "slike/znak" @itr ".jpg"))
                 [_ _ x-max y-max] (find-min-max nulte-koordinate)
                 width (inc x-max)
                 height (inc y-max)
                 img (.getSubimage image 0 0 width height)]
