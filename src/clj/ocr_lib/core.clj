...
(def process-images-progress-bar-value (atom 0))

(def total-pixels-value (atom 0))
...
(def grayscale-contrast-progress-value (atom 0))

(def read-black-dots-progress-value (atom 0))

(def grouping-dots-progress-value (atom 0))

(def grouping-all-dots (atom 0))

(def grouping-processed-dots (atom 0))

(def read-signs-progress-value (atom 0))

(def matching-progress-value (atom 0))
 ...
 (let [grayscale-contrast (if (> @total-pixels-value
  ...
  (when (< @matching-progress-value
   ...
   (reset!
     ...
     matching))
  (int
    (+ (* @grayscale-contrast-progress-value 0.07)
       (* @read-black-dots-progress-value 0.09)
       (* @grouping-dots-progress-value 0.46)
       (* @read-signs-progress-value 0.02)
       (* @matching-progress-value 0.36))
   ))
 )
...
(defn read-base64-image-fn
 ...
 [image-base64]
 (let [image-byte-array (.decode base64-decoder image-base64)
